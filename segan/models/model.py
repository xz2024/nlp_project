...
from persistentHomology1D.topo import q_distance
...
class SEGAN(Model):
    ...
    def __init__(self, opts, name='SEGAN',
        ...
        if discriminator is None:
            ...
            self.D = Discriminator(2, opts.denc_fmaps,
                                   dkwidth,
        ...
        # print('------------ Finish __init__ in SEGAN class. ------------')
    ...
    def infer_D(self, x_, ref):
        ...
        print("---------> D_in.shape: ", D_in.shape)    #->  D_in.shape:  torch.Size([100, 2, 16384])
        return self.D(D_in)
    ...
    def train(self, opts, dloader, criterion, l1_init, l1_dec_step,
        ...
        # print("l1_init: ", l1_init) # l1_init:  100
        # print("l1_dec_epoch: ", l1_dec_epoch)   # l1_dec_epoch:  100
        # print("l1_dec_step: ", l1_dec_step) # l1_dec_step:  1e-05
        ...
        print('---------------- model structure in L285 segan/models/model.py -----------------')
        # print(self.D)
        print(self.G)
        print('---------------- model structure in L288 segan/models/model.py -----------------')
        ...
        # debugging("271 Starting epoch...", 'model.py')
        for epoch in range(1, opts.epoch + 1):
            beg_t = timeit.default_timer()
            self.G.train()
            self.D.train()

            # debugging("279 Starting epoch...", 'model.py')
            # bidx 是第几批，batch 是一批的数据。
            for bidx, batch in enumerate(dloader, start=1):
                # debugging("281 Starting epoch...", 'model.py')
                # debugging(283, 'model.py')  #----------------------------------------------------
                ...
                sample = batch
                if len(sample) == 5:
                    uttname, clean, noisy, noisy_pre, slice_idx = batch
                    # print("uttname: ", type( uttname )) #-> <class 'list'>
                    # print("clean: ", type(clean))   #-> <class 'torch.Tensor'>
                    clean_numpy = clean.numpy()
                    noisy_pre_numpy = noisy_pre.numpy()
                    r, c = clean_numpy.shape
                    print('L321 ', clean_numpy.shape)   #-> (100, 16384)
                    print('L322 ', noisy_pre_numpy.shape) #->  (100, 16384)
                    g_topo_loss = q_distance(clean_numpy, noisy_pre_numpy, col = int(c/20))
                    g_topo_loss = torch.tensor( g_topo_loss )
                    g_topo_loss = g_topo_loss.to(device)
                    # print("g_topo_loss: ", g_topo_loss)
                ...
                total_d_real_loss = 0
                # print(type(noisy))
                Genh = self.infer_G(noisy_pre, clean)  # 从 noisy 生成 Genh data。
                lab = label
                # print(type(lab))    #-> <class 'torch.Tensor'>
                # print(lab)  # -> label全是 1。


                debugging("311 D real data...", 'model.py')
                # (1) D real data update
                d_real, _ = self.infer_D(clean, noisy)  # 从 clean data 预测出来的结果 d_real.
                d_real_loss = criterion(d_real.view(-1), lab)
                d_real_loss.backward()  # 反向传播 d_real_loss
                total_d_real_loss += d_real_loss

                debugging("318 D fake data...", 'model.py')
                # (2) D fake update
                d_fake, _ = self.infer_D(Genh.detach(), noisy) # 从 Genh data 预测出来的结果 d_fake.
                lab = label.fill_(0)    # -> label全是 0。
                d_fake_loss = criterion(d_fake.view(-1), lab)
                d_fake_loss.backward()  # 反向传播 d_fake_loss
                total_d_fake_loss += d_fake_loss
                Dopt.step()

                # d_loss = d_fake_loss + d_real_loss

                debugging("329 G fake data...", 'model.py')
                # (3) G real update
                Gopt.zero_grad()
                lab = label.fill_(1)
                # print("lab: ", type(lab))

                d_fake_, _ = self.infer_D(Genh, noisy)  # Genh is generaated data from G,
                g_adv_loss = criterion(d_fake_.view(-1), lab)
                g_l1_loss = l1_weight * F.l1_loss(Genh, clean)
                # print(g_l1_loss)    # tensor(14.5686, grad_fn=<MulBackward0>)
                # g_l1_loss += 3.1
                # print(g_l1_loss)
                # print("g_l1_loss: ", g_l1_loss)
                # print("g_adv_loss: ", g_adv_loss)

                # label = torch.ones(opts.batch_size)
                # label = label.to(device)
                # testing = torch.tensor(1.1)
                # g_l1_loss = g_l1_loss + testing
                # print(testing)
                # print(g_l1_loss)    # tensor(15.5686, grad_fn=<AddBackward0>)


                g_loss = g_adv_loss + g_l1_loss + g_topo_loss
                # print(type(g_loss)) #-> <class 'torch.Tensor'>
                print("g_topo_loss: ", g_topo_loss)
                print("g_loss: ", g_loss)  # -> tensor(431.7959, grad_fn=<AddBackward0>)
                ...
                # debugging("before 1st if", 'model.py')
                ...
                # debugging("before 2nd if", 'model.py')
                if bidx % log_freq == 0 or bidx >= len(dloader):
                    ...
                    # global
                    # debugging("387 Executing model_weights_norm()", 'model.py')
                    ...
                    # debugging("405 if not opts.no_train_gen: ", 'model.py')
                    ...
                    # debugging("409 Finish 2nd if", 'model.py')
                iteration += 1
                # debugging("411 Finish the "+ str(bidx) + " batch", 'model.py')  #----------------------------------------------------------------
            # debugging("412 Finish the last batch", 'model.py')
            # time.sleep(5)
        ...
        debugging("Finish Epoch", 'model.py')
